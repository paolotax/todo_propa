class Riga < ActiveRecord::Base
  ...
  scope :di_questa_propaganda,  joins(:appunto).where("appunti.created_at > ?", Date.new(2014,5,1))
  
  scope :di_quest_anno,         joins(:fattura).where("fatture.created_at > ?", Date.new(2014,5,1))
