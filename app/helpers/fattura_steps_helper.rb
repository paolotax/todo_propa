  ...
  def nested_righe_per_appunto(righe)
    ...
    righe.group_by(&:appunto).map do |a, righe|
      content_tag :div, :class => 'dettaglio_fattura' do
        content_tag( :div, check_box_tag("appunti_ids[]", a.id, false, {:class => 'cb-element'}) + link_to( "Ordine del #{l a.created_at, :format => :short} - copie #{a.totale_copie} - Importo #{a.totale_importo}", appunto_path(a)).html_safe, :class => 'consegna') +
